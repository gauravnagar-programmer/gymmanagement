...
const ActiveMembers = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (s === "expired in three days") return "bg-orange-700";
      ...
      <div className="md:ml-50 ml-30 h-screen space-y-5  px-5 py-10 ">
        ...
        <div className="grid-cols-1  grid md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-7">
          {activeMember.map((data, index) => (
            ...
            >
              <p className={`absolute top-4 font-semibold text-white  left-2 lowercase bg-green-500 rounded-full px-2 py-1  text-xs ${getStatusColor(data?.status)}`}>
                {data?.status}
              </p>
              <h3 className="mt-10">
