...
const MemberList = () => {
  ...
  const getStatusColor = (status = "") => {
    ...
    if (s === "expired in four to seven days") return "bg-yellow-500";
    if (s === "expired in three days") return "bg-orange-500";
      ...
      <div className="md:ml-50 ml-30 h-screen space-y-5  px-5 py-10 ">
        ...
        <div className="grid-cols-1 grid md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-7">
          {Data.map((data, index) => (
            <div
              key={index}
              className="flex relative  bg-gray-200 dark:bg-gray-700 flex-col space-y-3 text-gray-700 dark:text-white px-3 py-2 rounded-sm "
            >
              <p
                className={`absolute capitalize top-4 font-semibold text-white left-2  rounded-full px-2 py-1  text-xs ${getStatusColor(
                  data?.status
                )}`}
              >
                {data?.status}
              ...
              <div className="flex items-center md:px-5  py-3 justify-between">
                ...
                <button
                  ...
                  className="px-2 py-2 rounded-lg bg-red-500 cursor-pointer active:scale-90 md:bg-red-600 font-bold  md:font-semibold text-white"
                >
                  Delete
                </button>
              </div>
