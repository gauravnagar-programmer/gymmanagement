...
const SevenDays = () => {
  const [expiredMember7Days,setExpiredMember7Days] = useState([]);
  ...
  const getData = async () => {
    try {
      ...
      const filteredData =  res.data.filter(item => item.status == "expired in four to seven days")
      
      setExpiredMember7Days(filteredData);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="md:ml-50 ml-30 h-screen space-y-5  px-5 py-10 ">
        ...
        <p>Total Members : {expiredMember7Days.length}</p>
        <div></div>
        <div className="grid-cols-1  grid md:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-7">
          {expiredMember7Days.map((data, index) => (
            <div
              key={index}
            ...
            >
                <p className=" capitalize absolute top-4 font-semibold text-white  left-2  bg-yellow-500 rounded-full px-2 py-1  text-xs">{data?.status}</p>
              <h3 className="mt-10">
